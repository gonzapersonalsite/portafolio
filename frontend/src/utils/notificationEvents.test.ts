...
describe('notificationEvents', () => {
  ...
  it('returns unsubscribe function that stops receiving events', () => {
    ...
    unsubscribe()
    notificationEvents.emit('After unsubscribe')

    expect(listener).not.toHaveBeenCalled()
  })

  it('does not call unsubscribed listeners while keeping others', () => {
    const fn1 = vi.fn()
    const fn2 = vi.fn()
  ...
  it('continues calling remaining listeners when one throws', () => {
    const goodFn = vi.fn()
    const badFn = vi.fn(() => { throw new Error('Subscriber error') })

    // goodFn inserted first, so it runs before badFn throws
    notificationEvents.subscribe(goodFn)
    notificationEvents.subscribe(badFn)

    expect(() => notificationEvents.emit('Error in one')).toThrow('Subscriber error')
