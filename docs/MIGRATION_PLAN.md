...
**Estado:** CONGELADO tras verificación contra el código (2026-09-11)
**Rama de trabajo:** `refactor/static-migration`
**Reglas de ejecución:**
- NO se hace commit ni push durante la ejecución (solo al final, si el propietario lo decide).
- El plan puede modificarse durante la marcha **solo si hay justificación**; cada cambio debe registrarse en la sección "Modificaciones durante la ejecución".
...
- 1 perfil (`profiles`), 5 proyectos (`projects`), 33 skills (`skills`), 4 experiencias (`experiences`), 3 idiomas hablados (`spoken_languages`), 1 usuario admin (`users`, **no se migra**).
...
- [x] Copiar el dump a una ubicación externa al repo: `C:\Users\Gonzalo\Desktop\development\my-personal-projects\_backups\portafolio\` (sobrevivirá al borrado de `backend/`).
...
- [ ] Script de extracción **desechable** (Node, sin dependencias, en carpeta temporal fuera del repo) que parsea los bloques `COPY … FROM stdin;` del `.sql` (formato: tab-separado, `\N` = NULL, `\\` = `\`, `\.` = fin) y genera los JSON.
- [ ] Salidas (una por entidad): `src/entities/profile/api/data.json`, `src/entities/project/api/data.json`, `src/entities/skill/api/data.json`, `src/entities/experience/api/data.json`, `src/entities/spoken-language/api/data.json`.
- [ ] Mapeo 1:1 con los tipos TS existentes (`model/types.ts` de cada slice). Campos bilingües En/Es tal cual. `end_date` nulo → `undefined` (experiencia "En búsqueda activa").
- [ ] Deduplicar tecnologías preservando orden de primera aparición. Ordenar por `display_order`/`image_order`.
- [ ] Slugs: `licence-generator`, `pokedex`, `nutrimanager`, `portfolio`, `kanban-board` (mapeo slug↔UUID explícito en el script).
- [ ] `liveUrl` del proyecto Portfolio pasa de `./` a la URL real (Fase 5).
- [ ] Verificación: test unitario de esquema (Fase 3) + comparativa visual en `pnpm dev` contra producción.
...
- [ ] Verificación: `grep -r "postimg\|placehold\|drive.google"` en `src/` y JSONs → 0 resultados (excepto CV de Google Drive, que se mantiene como enlace externo deliberado).
...
- [ ] Eliminar `backend/` completo (historia preservada en git + tag Fase 0).
- [ ] Eliminar: `docker-compose.yml`, `frontend/Dockerfile`, `frontend/nginx.conf`, `.env` y `.env.production` raíz. Crear `frontend/.env.local` (gitignored) con las 3 `VITE_EMAILJS_*` para desarrollo local.
- [ ] `frontend/vercel.json`: eliminar rewrite `/api` y bloque headers `/api`; conservar SPA rewrite y headers de assets.
- [ ] `frontend/vite.config.ts`: eliminar bloque `server.proxy['/api']`.
- [ ] `.github/workflows`: eliminar `backend-ci.yml`; `frontend-ci.yml` se mantiene.
- [ ] GitHub (manual por el propietario): eliminar secret `RENDER_DEPLOY_HOOK_URL`.
- [ ] Documentación: reescribir `README.md`, `ARCHITECTURE.md`, `OPERATIONS.md`, `docs/es/*` (arquitectura estática, sin Render/Aiven/Swagger/JWT), simplificar `AGENTS.md` raíz (ya no es monorepo) y eliminar `.agents/skills/monorepo-router/`.
- [ ] `.gitignore`: limpiar entradas de backups/sql que ya no apliquen.
- [ ] Vercel (manual por el propietario): eliminar env var `VITE_API_BASE_URL`; mantener `PNPM_APPROVE_BUILDS=true`.
...
- [x] Local: `pnpm lint`, `pnpm test` (26 tests), `pnpm build` en `frontend/` — cero errores.
...
- [ ] Deploy de preview de Vercel (rama) → smoke test en URL de preview (manual del propietario, o al merge).
- [ ] Merge a `main` (cuando el propietario lo autorice) → Vercel despliega. Verificar en producción: cero llamadas a dominios externos salvo EmailJS.
- [ ] Periodo de gracia 1 semana → después: apagar y eliminar servicio Render y base Aiven (dump archivado en `_backups/portafolio/`).
...
## Pasos manuales pendientes del propietario
...
3. **Merge:** autorizar merge de `refactor/static-migration` → `main` y verificar producción (cero errores de consola salvo EmailJS).
4. **Periodo de gracia:** tras 1 semana con el estático en producción, eliminar el servicio de Render y la base de datos de Aiven (dump archivado en `C:\Users\Gonzalo\Desktop\development\my-personal-projects\_backups\portafolio\backup_20260911_063742.sql`).
5. **Opcional:** actualizar capturas del proyecto "Portfolio" si se desea que muestren el sitio nuevo.
